...
static const char* TAG = "D7SSensor";

RAK_D7S::RAK_D7S(byte addr) : TelemetrySensor(meshtastic_TelemetrySensorType_D7S, "D7S")
{
  _deviceAddress = addr;
}
...
uint8_t RAK_D7S::isInCollapse() 
{
  ...
  return (_events & 0x02) >> 1; // Return the second bit of _events.
}
...
uint8_t RAK_D7S::isInShutoff()
{
  ...
  return _events & 0x01; // Return the second bit of _events.
}
...
void RAK_D7S::setup() {}
// Run Once
int32_t RAK_D7S::runOnce()
{
while(begin()){
  LOG_DEBUG(".");
};
LOG_DEBUG("D7S::runOnce begin ok");
setAxis(SWITCH_AT_INSTALLATION);
LOG_DEBUG("D7S Initializing keep the device steady!");
initialize();
while(!isReady()){
  LOG_DEBUG("*");
}
status = isReady();
LOG_DEBUG("OMRON D7S INITIALIZED");
return initI2CSensor();
}

// Getting metrics for meshtastic
bool RAK_D7S::getMetrics(meshtastic_Telemetry *measurement){
  LOG_DEBUG("D7S getmetrics id: %i", measurement->which_variant);

  //if(isEarthquakeOccuring() == 1){
    float lastest_si = getLastestSI(0);
    float lastest_pga = getLastestPGA(0);
    uint8_t collapse_flag = isInCollapse();
    LOG_DEBUG("D7S getMetrics SI: %.3fm/s PGA: %.3fm/s^2 collapse_flag: %d",lastest_si,lastest_pga,collapse_flag);
    
    measurement->variant.environment_metrics.has_si_value = true;
    measurement->variant.environment_metrics.has_pga_value = true;
    measurement->variant.environment_metrics.has_collapse_flag = true;

    measurement->variant.environment_metrics.si_value = lastest_si;
    measurement->variant.environment_metrics.pga_value = lastest_pga;
    measurement->variant.environment_metrics.collapse_flag = collapse_flag;
